...
impl Grapheme<'_> {
    ...
    // A grapheme cluster is considered printable if it is a private-use character or its display
    // width is non-zero per UCS and UAX11. Private-use characters have no display width, but are
    // typically rendered as a replacement glyph when not assigned or recognized. When used as
    // intended, they represent a glyph with some non-zero rendered width.
...
mod tests {
    ...
    #[rstest]
    fn empty_str_has_no_printable_text() {
        assert!(!"".has_printable_text());
    }
    ...
    fn str_with_private_use_chars_has_printable_text(#[case] text: &str) {
        assert!(text.has_printable_text())
    }
}
